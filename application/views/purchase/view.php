            ...
            <div id="invoice-template" class="card-body">
                        ...
                        if ($invoice['status'] != 'canceled') { ?>
                            <div class="title-action">
                                ...
                                <a href="#part_payment" data-toggle="modal" data-remote="false" data-type="reminder"
                                    class="btn btn-large btn-success" title="Partial Payment"><span class="fa fa-money"></span> <?php echo $this->lang->line('Make Payment') ?> </a>
                                ...
                                <a href="#pop_model" data-toggle="modal" data-remote="false"
                                    class="btn btn-large btn-success" title="Change Status"><span class="fa fa-retweet"></span> <?php echo $this->lang->line('Change Status') ?></a>
                ...
                <div id="invoice-company-details" class="row mt-2">
                    ...
                    <div class="col-md-6 col-sm-12 text-xs-center text-md-right">
                        ...
                        <h2>Commercial Invoice No</h2>
                        <p class="pb-1"><?php echo $invoice['commercial_invoice_no'] ?> </p>
                        <h2>Performa Invoice No</h2>
                        <p class="pb-1"><?php echo $invoice['proforma_invoice_no'] ?> </p>
                        <ul class="px-0 list-unstyled">
                            <li><?php echo $this->lang->line('Gross Amount') ?></li>
                            <li class="lead text-bold-800"><?php echo $invoice['currency_code'] ?><?php echo amountExchange($invoice['total'], 0, $this->aauth->get_user()->loc) ?></li>
                        </ul>
                    </div>
                </div>
                <!--/ Invoice Company Details -->
                                ...
                                <thead>
                                    <?php if ($invoice['taxstatus'] == 'cgst') { ?>
                                        ...
                                        <tr>
                                            ...
                                            <th class="text-xs-left">Shipping</th>
...
<th scope="row">' . $c . '</th>
                            <td>' . $row['code'] . '</td>  
                            <td>' . $row['product'] . '</td>  
                            <td>' .  $invoice['currency_code'] . amountExchange($row['price'], 0, $this->aauth->get_user()->loc) . '</td>
                             <td>' . amountFormat_general($row['qty']) . $row['unit'] . '</td>
                              <td>' . $invoice['currency_code'] . amountExchange($row['totaldiscount'], 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($row['discount']) . $this->lang->line($invoice['format_discount']) . ')</td>
                            <td>' . ' ' . amountFormat_s($row['shipping']) . '</td>
                           
                              <td>' . $invoice['currency_code'] . amountExchange($gst, 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($rate) . '%)</td>
                             <td>' . $invoice['currency_code'] . amountExchange($gst, 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($rate) . '%)</td>                           
                            <td>' . $invoice['currency_code'] . amountExchange($row['subtotal'], 0, $this->aauth->get_user()->loc) . '</td>
                        </tr>';
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . ' ' . amountFormat_s($row['shipping']) . '</td>
                              ...
                              <td>' . $invoice['currency_code'] . amountExchange($row['totaltax'], 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($row['tax']) . '%)</td>
                                ...
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="text-xs-right"><?php echo $invoice['currency_code'] ?> <?php echo amountExchange($sub_t, 0, $this->aauth->get_user()->loc) ?></td>
                                        </tr>
                                        <tr>
                                            <td><?php echo $this->lang->line('TAX') ?></td>
                                            <td class="text-xs-right"><?php echo $invoice['currency_code'] ?> <?php echo amountExchange($invoice['tax'], 0, $this->aauth->get_user()->loc) ?></td>
                                        </tr>
                                        <tr>
                                            <td><?php echo $this->lang->line('Discount') ?></td>
                                            <td class="text-xs-right"><?php echo $invoice['currency_code'] ?> <?php echo amountExchange($invoice['discount'], 0, $this->aauth->get_user()->loc) ?></td>
                                        </tr>
                                        <tr>
                                            <td><?php echo $this->lang->line('Shipping') ?></td>
                                            <td class="text-xs-right"><?php echo $invoice['currency_code'] ?> <?php echo amountExchange($invoice['shipping'], 0, $this->aauth->get_user()->loc) ?></td>
                                        </tr>
                                        <tr>
                                            <td class="text-bold-800"><?php echo $this->lang->line('Total') ?></td>
                                            <td class="text-bold-800 text-xs-right"><?php echo $invoice['currency_code'] ?> <?php echo amountExchange($invoice['total'], 0, $this->aauth->get_user()->loc) ?></td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr class="bg-grey bg-lighten-4">
                                            ...
                                            <td class="text-bold-800 text-xs-right"><?php echo $invoice['currency_code'] ?> <?php $myp = '';
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="input-group">
                                <div class="input-group-addon"><?php echo $invoice['currency_code'] ?></div>
