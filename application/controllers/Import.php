    ...
    public function start_process()
    {
        ...
        foreach ($sheetData as $row) {
            ...
            $products[] = array(
                ...
                'pcat' => $pcat,
                'warehouse' => $warehouse,
                'product_name' => mb_convert_encoding($row[0], 'UTF-8', 'auto'), // Ensure UTF-8 encoding
                'product_code' => $row[1],
                'product_price' => $row[2],
                ...
                'disrate' => $row[5],
                'qty' => $row[6],
                'product_des' => mb_convert_encoding($row[7], 'UTF-8', 'auto'), // Ensure UTF-8 encoding
                'alert' => $row[8],
                'barcode' => $barcode
            );
        }
...
//print_r($sheetData);
    ...
    function accounts(): void
    {
        ...
        $data['warehouse'] = $this->categories_model->warehouse_list();
        $this->load->view('fixed/header', $head);
        $this->load->view('import/accounts', $data);
        $this->load->view('fixed/footer');

    }


    public function accounts_upload()
    {
        ...
        $this->load->view('import/accounts-wizard', $data);

        $this->load->view('fixed/footer');

    }

    public function accounts_start_process()
    {
        require APPPATH . 'third_party/vendor/autoload.php';
    
        $name = $this->input->post('name');
        log_message('error', 'Filename received: ' . $name);
    
        $inputFileName = FCPATH . 'userfiles/' . $name;
    
        if (!file_exists($inputFileName)) {
            echo json_encode(['status' => 'Error', 'message' => "File not found: $inputFileName"]);
            return;
        }
        ...
        try {
            ...
            echo json_encode(['status' => 'Error', 'message' => 'Failed to read Excel file: ' . $e->getMessage()]);
            return;
        }
        ...
        $allowedTypes = ['Assets', 'Expenses', 'Income', 'Liabilities', 'Equity', 'Basic'];
        $data = [];
        $invalidRows = [];
        $duplicateRows = [];
    
        // Get existing acn values from DB
        $existingAcns = $this->db->select('acn')->from('geopos_accounts')->get()->result_array();
        $existingAcns = array_column($existingAcns, 'acn');
    
        foreach ($sheetData as $index => $row) {
            $acn = trim($row[0]);
            $accountType = trim($row[3]);
            ...
            if (in_array($acn, $existingAcns)) {
                $duplicateRows[] = $index + 2;
                continue;
            }
        ...
        // Check and show errors for duplicates
        if (!empty($duplicateRows)) {
            echo json_encode([
                'status' => 'Error',
                'message' => 'Duplicate ACNs found in rows: ' . implode(', ', $duplicateRows)
            ]);
            return;
        }
    
        // Check and show errors for invalid types
        if (!empty($invalidRows)) {
            echo json_encode([
                'status' => 'Error',
                'message' => 'Invalid account types found in rows: ' . implode(', ', $invalidRows)
            ]);
            return;
        }
        ...
        if (!empty($data)) {
            ...
            $inserted = $this->db->insert_batch('geopos_accounts', $data);
    
            if ($inserted) {
                echo json_encode(['status' => 'Success', 'message' => "Accounts imported successfully!"]);
            } else {
