...
class Products extends CI_Controller
{
    ...
    public function product_list()
    {
        ...
        foreach ($list as $prd) {
            ...
            // Fetch all purchase prices, shipping, and their currency codes for the product
            $this->db->select('geopos_purchase_items.price, geopos_purchase.currency_code, geopos_purchase_items.qty, geopos_purchase_items.shipping');
            $this->db->from('geopos_purchase_items');
            $this->db->join('geopos_purchase', 'geopos_purchase.id = geopos_purchase_items.tid', 'left');
            $this->db->where('geopos_purchase_items.pid', $pid);
            $query = $this->db->get();
            $purchase_items = $query->result_array();
    
            $total_price_in_sar = 0;
            $count = 0;
    
            // Convert prices and shipping to SAR and calculate the total
            foreach ($purchase_items as &$item) {
                $price = $item['price'];
                $shipping = isset($item['shipping']) ? $item['shipping'] : 0; // Ensure shipping is set
                $currency_code = $item['currency_code'];
                $quantity = isset($item['qty']) && $item['qty'] > 0 ? $item['qty'] : 1; // Avoid division by zero
    
                // Convert price to SAR
                if ($currency_code !== 'SAR') {
                    $price_in_sar = $this->convert_to_sar($price, $currency_code);
                } else {
                    $price_in_sar = $price;
                }
    
                // Convert shipping to SAR
                if ($currency_code !== 'SAR') {
                    $shipping_in_sar = $this->convert_to_sar($shipping, $currency_code);
                } else {
                    $shipping_in_sar = $shipping;
                }
    
                // Add shipping cost per unit to the price
                $price_in_sar += $shipping_in_sar / $quantity;
    
                $total_price_in_sar += $price_in_sar;
                $count++;
            }
    
            // Calculate the average price in SAR
            $average_purchase_price = ($count > 0) ? ($total_price_in_sar / $count) : 0;
            ...
            $row[] = $prd->c_title;
            $row[] = $prd->title;
            $row[] = amountExchange($average_purchase_price, 0, $this->aauth->get_user()->loc) * 1.5;
            $row[] = amountExchange($average_purchase_price, 0, $this->aauth->get_user()->loc); // Add average purchase price
            if ($catid > 0) {
                $row[] = amountExchange($prd->qty * $prd->product_price);
            }
...
public function edit()
{
    ...
    // Fetch all purchase prices and their currency codes by joining geopos_purchase_items and geopos_purchase
    $this->db->select('geopos_purchase_items.price, geopos_purchase.currency_code, geopos_purchase_items.qty, geopos_purchase_items.shipping');
    $this->db->from('geopos_purchase_items');
    $this->db->join('geopos_purchase', 'geopos_purchase.id = geopos_purchase_items.tid', 'left');
    $this->db->where('geopos_purchase_items.pid', $pid);
    ...
    // Convert prices and shipping to SAR if necessary and add converted values to the array
    foreach ($purchase_items as &$item) {
        $price = $item['price'];
        $shipping = isset($item['shipping']) ? $item['shipping'] : 0; // Ensure shipping is set
        $currency_code = $item['currency_code'];
        ...
        // Replace the original price with the converted price in SAR
        $item['price'] = $price_in_sar;
    ...
    // Calculate the average price in SAR
    $average_price = ($count > 0) ? ($total_price_in_sar / $count) : 0;
    $data['product']['fproduct_price'] = $average_price;

    // Pass purchase prices (converted to SAR) to the view
    $data['purchase_prices'] = $purchase_items;
    ...
    // Set page title and user info
    $head['title'] = "Edit Product";
    $head['usernm'] = $this->aauth->get_user()->username;

    // Load variables for units
    $this->load->model('units_model', 'units');
    $data['variables'] = $this->units->variables_list();
    ...
    //     // Convert prices to SAR if necessary and add converted prices to the array
    //     foreach ($purchase_items as &$item) {
    //         $price = $item['price'];
    //         $currency_code = $item['currency_code'];

    //         if ($currency_code !== 'SAR') {
    //             // Convert to SAR using the conversion function
    //             $price_in_sar = $this->convert_to_sar($price, $currency_code);
    //         } else {
    //             $price_in_sar = $price;
    //         }
    ...
    //     $this->load->view('fixed/footer');
    // }
    private function convert_to_sar($price, $currency_code)
    {
        // Define conversion rates (example rates, replace with actual rates or API calls)
        $conversion_rates = [
        ...
        if (isset($conversion_rates[$currency_code])) {
            return $price * $conversion_rates[$currency_code];
        }

        // If the currency is not in the list, return the original price (or handle as needed)
        return $price;
    }
    ...
    //     // Fetch all purchase prices for the product
    //     $this->db->select('price');
    //     $this->db->from('geopos_purchase_items');
    //     $this->db->where('pid', $pid);
    //     $query = $this->db->get();
    //     $data['purchase_prices'] = $query->result_array();
    ...
    //     $this->db->where('pid', $pid);
    //     $query = $this->db->get();
    //     $avg_price_result = $query->row_array();
    //     $data['product']['fproduct_price'] = $avg_price_result['avg_price'] ?? 0; // Default to 0 if no purchases
