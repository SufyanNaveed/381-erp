...
class Purchase extends CI_Controller
{
    ...
    {
        // echo $this->input->post('currency_code');exit;
        ...
        $commercialInvoiceNo = $this->input->post('commercial_invoice_no');
        $performaInvoiceNo = $this->input->post('performa_invoice_no');
   ...
   'proforma_invoice_no'=>$performaInvoiceNo );
        ...
        if ($this->db->insert('geopos_purchase', $data)) {
            ...
            foreach ($pid as $key => $value) {
                ...
                $total_shipping += numberClean($product_shipping[$key]);
            ...
            if ($prodindex > 0) {
                ...
                $this->db->set(array('shipping'=>$total_shipping,'discount' => rev_amountExchange_s(amountFormat_general($total_discount), $currency, $this->aauth->get_user()->loc), 'tax' => rev_amountExchange_s(amountFormat_general($total_tax), $currency, $this->aauth->get_user()->loc), 'items' => $itc));
    ...
    {
        // print_r($this->input->post('restock'));exit;
        $currency = $this->input->post('mcurrency');
        $customer_id = $this->input->post('customer_id');
        ...
        $performaInvoiceNo = $this->input->post('performa_invoice_no');
        if ($currencyCode) {
            $this->db->set('currency_code', $currencyCode);
            $this->db->set('commercial_invoice_no',$commercialInvoiceNo);
            $this->db->set('proforma_invoice_no',$performaInvoiceNo);
            $this->db->set('multi', $currencyCode);
            $this->db->where('id', $invocieno);
            $this->db->update('geopos_purchase');
        }
        //Product Data
        ...
        foreach ($pid as $key => $value) {
            ...
            if ($this->input->post('update_stock') == 'yes') {
                ...
                $qty_diff = numberClean(@$product_qty[$key]) - numberClean(@$old_product_qty[$key]);

                // If quantity difference is zero, still update the stock
                if ($qty_diff == 0) {
                    $qty_diff = numberClean(@$product_qty[$key]); // Use the current quantity
                }
        ...
        $data = array('invoicedate' => $bill_date, 'invoiceduedate' => $bill_due_date, 'subtotal' => $subtotal, 'shipping' => $total_shipping, 'ship_tax' => $shipping_tax, 'ship_tax_type' => $ship_taxtype, 'discount' => $total_discount, 'tax' => $total_tax, 'total' => $total, 'notes' => $notes, 'csd' => $customer_id, 'items' => $itc, 'taxstatus' => $tax, 'discstatus' => $discstatus, 'format_discount' => $discountFormat, 'refer' => $refer, 'term' => $pterms, 'multi' => $currency);
        $this->db->set($data);
        $this->db->where('id', $invocieno);
