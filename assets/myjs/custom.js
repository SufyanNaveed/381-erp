...
$('#addproduct').on('click', function () {
    ...
    var currencyCode = $('#currencyCode').val(); // ✅ Get selected currency
    ...
    var data = '<tr><td><input type="text" class="form-control" name="product_code[]" placeholder="Enter Product Code" id="productcode-' + cvalue + '"></td><td><input type="text" class="form-control" name="product_name[]" placeholder="Enter Product name" id="productname-' + cvalue + '"></td>'
        ...
        + '<td><input type="text" class="form-control shipping" name="product_shipping[]" onkeypress="return isNumber(event)" id="shipping-' + cvalue + '" onkeyup="rowTotal(' + functionNum + '), billUpyog()" autocomplete="off"></td>'
        + '<td><span class="selectedCurrency">' + currencyCode + '</span> <strong><span class=\'ttlText\' id="result-' + cvalue + '">0</span></strong></td>'
        ...
        + '</tr><tr><td colspan="8"><textarea class="form-control" id="dpid-' + cvalue + '" name="product_description[]" placeholder="Enter Product description" autocomplete="off"></textarea><br></td></tr>';
...
function updateCurrencyDisplay(value) {
    // let symbol = '';
    // switch(value) {
    //     case 'SAR':
    //         symbol = '﷼';
...
window.addEventListener('DOMContentLoaded', () => {
    const defaultCurrency = document.getElementById('currencyCode').value;
    updateCurrencyDisplay(defaultCurrency);
});
//caculations
...
var samanYog = function () {
    ...
    for (var z = 0; z < idList.length; z++) {
        ...
        var s1 = accounting.unformat($("#shipping-" + x).val(), accounting.settings.number.decimal);
        ...
        if (s1 > 0) {
            shippings += s1; // Add shipping cost
        }
    }
    ...
    $("#shippings").html(accounting.formatNumber(shippings)); // Correctly update shipping total
...
//     var result;
//     var totalValue = 0;
//     var amountVal = accounting.unformat($("#amount-" + numb).val(), accounting.settings.number.decimal); // Quantity
//     var priceVal = accounting.unformat($("#price-" + numb).val(), accounting.settings.number.decimal); // Product price (rate)
//     var discountVal = accounting.unformat($("#discount-" + numb).val(), accounting.settings.number.decimal); // Discount
//     var shippingVal = accounting.unformat($("#shipping-" + numb).val(), accounting.settings.number.decimal); // Shipping cost
//     var vatVal = accounting.unformat($("#vat-" + numb).val(), accounting.settings.number.decimal); // VAT
//     var taxo = 0;
//     var disco = 0;

//     // Ensure quantity is greater than 0 to avoid division by zero
//     if (amountVal > 0) {
//         // Update price based on the formula: rate + (shipping cost / qty)
//         priceVal = priceVal + (shippingVal / amountVal);
//         $("#price-" + numb).val(accounting.formatNumber(priceVal)); // Update the price in the DOM
//     } else {
...
var rowTotal = function (numb) {
    ...
    var shippingVal = accounting.unformat($("#shipping-" + numb).val(), accounting.settings.number.decimal);
...
//before tax
   ...
   // ➕ Add shipping cost before updating total
let shippingTotal = 0;
let shippingAmount = 0;
if (amountVal > 0) {
    shippingTotal = shippingVal / amountVal;
    shippingAmountTotal = shippingAmount / amountVal;
    shippingTotal = shippingTotal * amountVal;
}

// priceVal += shippingTotal;
//     console.log('shippingTotal',shippingTotal);
//     console.log('totalPrice',priceVal);
    // $("#price-" + numb).val(priceVal);

totalValue = totalValue + shippingTotal;
...
function formatRest(taxFormat, disFormat, trate = '') {
    ...
    for (var i = 0; i < idArray.length; i++) {
        ...
        // Add shipping cost to the result
        result += shippingVal;
    ...
    $("#shippings").html(accounting.formatNumber(shippingTotal)); // Update total shipping cost
    billUpyog();
}
